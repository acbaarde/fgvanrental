...
foreach($result as $row){
    ...
    $pdf->Text($x,$y+40,"MANUAL TRIPS");
    $pdf->Text($x + 68,$y+40,$row['manualtrips']);
    $pdf->SetFont('Times','',11);

    $pdf->Text($x,$y+45,"TOTAL AMOUNT");
    $pdf->SetFont('Times','B',11);
    $pdf->Text($x + 68,$y+45,$this->mylib->num_decimal($row['manualtripsamount'])); // TOTAL AMOUNT
    $pdf->Text($x + 68,$y+82,$this->mylib->num_decimal($row['manualexpenses'])); // TOTAL EXPENSES

    $pdf->SetFont('Times','',11);
    $pdf->Text($x,$y+55,"EXPENSES:");

    $pdf->Text($x + 2,$y+60,"10% TAX");
    $pdf->Text($x + 68,$y+60,$this->mylib->num_decimal($row['manualtrips_tax_10'],2,9));

    $pdf->Text($x + 2,$y+65,"3% ANNUAL TAX");
    $pdf->Text($x + 68,$y+65,$this->mylib->num_decimal($row['manualtrips_tax_3']));

    $pdf->Text($x + 2,$y+70,"ADMIN FEE");
    $pdf->Text($x + 36,$y+70,"0.00");
    $pdf->Text($x + 52,$y+70,"");
    $pdf->Text($x + 68,$y+70,"0.00");

    $pdf->Text($x + 2,$y+75,"OTHER FEE(s)");
    $pdf->Text($x + 68,$y+75,"0.00");
    ...
    $pdf->Text($x + 68,$y+92,$this->mylib->num_decimal($row['manualnet']));
    ...
    $pdf->Text($x + 2,$y+60,"10% TAX");
    $pdf->Text($x + 68,$y+60,$this->mylib->num_decimal($row['manualtrips_tax_10']));
